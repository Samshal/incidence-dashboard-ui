...
export class HorizontalTimelineComponent implements AfterViewInit {
  ...
  @ViewChild('eventsWrapper') eventsWrapper: any;
  @ViewChild('fillingLine') fillingLine: any;
  @ViewChildren('timelineEvents') timelineEvents:any;

  eventsWrapperWidth: number = 0;
  private _viewInitialized = false;

  constructor(private _cdr: ChangeDetectorRef, private eventsService: EventsService) {
  }
  ...
  onEventClick(event: Event, selectedItem: TimelineElement) {
    ...
    this.eventsService.broadcast("timeline-element-selected", selectedItem);
